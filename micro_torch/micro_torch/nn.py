...
class Embedding(Module):
    ...
    def __init__(self, num_embeddings, embedding_dim, dtype=micro_torch.float64):
        self.weight = micro_torch.randn(num_embeddings, embedding_dim, dtype=dtype) * (num_embeddings)**-0.5
        self.dtype = dtype
        # grads
    ...
    def backward(self, grad):
        self.weight_grad = micro_torch.zeros_like(self.weight)
        self.weight_grad.index_add_(dim=0, index=self.x.view(-1), source=grad.view(-1, self.weight.shape[1]))
        return None

class Linear(Module):

    def __init__(self, in_features, out_features, bias=True, dtype=micro_torch.float64):
        self.weight = micro_torch.randn(in_features, out_features, dtype=dtype) * (in_features)**-0.5
        self.bias = micro_torch.zeros(out_features, dtype=dtype) * 0 if bias else None
        self.dtype = dtype
        # grads
        self.weight_grad = None
        self.bias_grad = None
...
class BatchNorm1d(Module):
    def __init__(self, in_features, eps=1e-5, momentum=0.001, dtype=micro_torch.float64): # manual bn need fp64
        self.weight = micro_torch.ones(in_features, dtype=dtype)
        self.bias = micro_torch.zeros(in_features, dtype=dtype)
        self.running_mean = micro_torch.zeros(in_features, dtype=dtype)
        self.running_var = micro_torch.ones(in_features, dtype=dtype)
        self.eps = eps
    ...
    def __call__(self, x):
        if self._training:
            ...
            mean = x.mean(dim=0)
            var = ((x - mean) ** 2).mean(dim=0) # as micro_torch, we don't use Bessel correction
...
class LayerNorm(Module):
    def __init__(self, in_features, eps=1e-5, dtype=micro_torch.float64):
        self.weight = micro_torch.ones(in_features, dtype=dtype)
        self.bias = micro_torch.zeros(in_features, dtype=dtype)
        self.eps = eps
        self.dtype = dtype
...
class CrossEntropyLoss(Module):
    ...
    def __call__(self, x, y):
        ...
        probs = exp_l / count
        loss = -probs[range(y.shape[0]), y.tolist()].log().mean()
        # backward buffer
        self.probs = probs
        self.y = y
        return loss
    ...
    def backward(self, grad):
        b = self.y.shape[0]
        x_grad = self.probs
        x_grad[range(b), self.y.tolist()] -= 1
